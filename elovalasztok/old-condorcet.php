    ...
    /* A Schulze method implementation*/
	  ...
	  */	
      ...
      public function report() {
		  ...
		  $this->showResult($this->shortlist);
          ...
          alternatívától az oszlopban lévőhöz vezető legerősebb Schulze method szerinti útvonal 
          "erejét" mutatja.</p>
          ';
          ...
          $result .= '<h3>Condorcet / Schulze  módszer szerinti sorrend</h3>';
          $result .= $this->showlist($this->shortlist).'</div>';
          return $result;
      }
      ...
      private function showResult($shortlist) {
          ...
          foreach($shortlist as $j => $i) {
			  if ($j == 0)
				  ...
				  $helyezes++;
			  $info = '';
			  if (count($shortlist) > ($j+1)) {
			    if (($j == 0) & ($values[$shotlist[$j+1]] < $values[$i]))
				   $info = 'Condorcet gyöztes';
			    else if ($values[$i] == $values[$shotlist[$j+1]])
				   $info = 'döntetlen';
			  }
			  if ($j > 0) {
